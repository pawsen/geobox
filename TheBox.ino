...
void setup()
{
    ...
    // if settingsfile is not found, make infinite loop. Prevent writing to eeprom.
    if(box.gamestate == gs_removePower)
        while(true){}
        ...
        */
        if(menu.changed || menu.id == 0){
            ...
            if (menu.id == 0){
                 ...
                 * showMission
                 */
                fileRead(PFFS.open_file(const_cast<char*>(MISSIONS)),MISSIONS,menu.id,&ms);
